...
The current `0.24.1` line uses Spring Boot 3.3 / Java 17+ as the mainline. The verified capabilities include cache, messaging, jobs, observation bridging, the local governance runtime, the read-only governance diagnostic Console, the Boot3 Sentinel provider, retry-stop propagation, isolation by fixed traffic class and priority, abnormal instance candidate detection, and local in-JVM fault traces. The governance platform now includes asset models, resource and signal ingestion APIs, service and dependency topology queries, a JDBC storage boundary, Console Platform Mode, incident evidence packages, request-flow queries, governance-plan dry-runs, notification previews, and the connector configuration center. `0.24.1` wires the governance platform sample to Dockerized Redis, Postgres, RabbitMQ, Prometheus, and SkyWalking so it can run real reads/writes, publish/consume checks, Prometheus scraping, and SkyWalking agent trace persistence. Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21 entries are still provided per verified module.

The `0.24.1` line is not a replacement for Sentinel, Spring Cloud Gateway, SkyWalking, Prometheus, enterprise IM, or an OpenTelemetry backend, and it does not change external tool rules. It puts Nexary SDK resources, governance signals, service groups, clusters, zones, middleware dependencies, local connector configuration, and read-only connector states into one platform view, then shows the primary resource, impacted resource count, evidence timeline, and suggested check for each incident candidate. The connector center can store local metadata for SkyWalking, Gateway, Sentinel, Prometheus, Actuator, and Feishu/DingTalk/Webhook, run explicit TEST / DRY-RUN probes, and maintain mappings from Nexary services to external tool objects; it still does not write Sentinel rules, change Gateway routes, or send production alerts. Payloads, URL queries, user ids, tenants, message ids, cache keys, exception text, stack traces, tokens, and passwords must not enter platform signals.
...
To try the read-only governance platform RC:
...
curl -s http://localhost:18092/api/platform/snapshot
...
`./scripts/console/up.sh` starts the local Console, Redis, Postgres, RabbitMQ, and Prometheus. `/demo/platform/probe` performs real local read/write probes against those components and records the results as request flows, transaction stats, and Prometheus metrics.

To inspect abnormal instance candidate detection:
...
- `nexary-governance/nexary-governance-runtime`: deadline, traffic, rate limit, bulkhead, degrade, retry-stop, priority isolation, instance health, and local fault trace primitives
...
The current source version is `0.22.0`. If Maven Central has not synced this version yet, build from GitHub tag `v0.22.0` and install it locally:
...
- Use a GitHub Releases / Tags version. For example, tag `v0.22.0` maps to dependency version `0.22.0`.
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | currently verified | `0.22.0`; use Latest Version after Maven Central sync | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter`<br>`nexary-governance-gateway-spring-boot-starter`<br>`nexary-governance-sentinel-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis single-tier, Messaging Redis-only, Job local/XXL-JOB/PowerJob bridge, and Gateway cancellation are verified | `0.22.0`; use Latest Version after Maven Central sync | current entries use direct versions; switch only when a dedicated BOM is released | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter`<br>`nexary-governance-gateway-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 primary validation runtime | Cache Redis, Messaging by provider, and Job local/XXL-JOB/PowerJob bridge are verified; this is not whole-repository Boot4 support | `0.22.0`; use Latest Version after Maven Central sync | current entries use direct versions; switch only when a dedicated BOM is released | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` plus one Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
<properties>
  ...
  <nexary.version>0.22.0</nexary.version>
</properties>
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot2-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot2-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot2-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-governance-gateway-spring-boot2-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot4-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot4-starter:0.22.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot4:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot4-starter:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-api:0.22.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot2:0.22.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot2:0.22.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot2:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot2:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot2:0.22.0'
}
```
...
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot4:0.22.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot4:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot4:0.22.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot4:0.22.0'
}
```
...
- `0.22.0` includes verified entries for Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21
