...
The current `0.22.0` line uses Spring Boot 3.3 / Java 17+ as the mainline. The verified capabilities include cache, messaging, jobs, observation bridging, the local governance runtime, the read-only governance diagnostic Console, the Boot3 Sentinel provider, retry-stop propagation, isolation by fixed traffic class and priority, abnormal instance candidate detection, and local in-JVM fault traces. The platform direction now includes asset models, resource and signal ingestion APIs, service and dependency topology queries, a Postgres storage boundary, Console Platform Mode, and the v0.18 incident evidence chain: slow calls, error rate signals, Sentinel blocks, Gateway disconnects, retry-stop signals, and abnormal instance signals from the same service, cluster, and zone are grouped into readable incident candidates. `0.22.0` wires the governance platform sample to Dockerized Redis, Postgres, RabbitMQ, Prometheus, and SkyWalking so it can run real reads/writes, publish/consume checks, Prometheus scraping, and SkyWalking agent trace persistence. Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21 entries are still provided per verified module.

The `0.22.0` line is not a replacement for Sentinel, Spring Cloud Gateway, SkyWalking, Prometheus, enterprise IM, or an OpenTelemetry backend, and it does not change external tool rules. It puts Nexary SDK resources, governance signals, service groups, clusters, zones, middleware dependencies, and read-only connector states into one platform view, then shows the primary resource, impacted resource count, evidence timeline, and suggested check for each incident candidate. Local Console remains for single-JVM debugging; Platform Mode is for aggregated services, dependencies, incident queue, topology impact, and evidence entry points. Payloads, URL queries, user ids, tenants, message ids, cache keys, exception text, stack traces, tokens, and passwords must not enter platform signals.
