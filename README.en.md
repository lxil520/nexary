...
The `0.11.0` line does not replace Sentinel and does not add a remote governance platform. Its narrow job is stale request cancellation: when a deadline has already expired, or when a request is no longer worth starting before it enters local governance, Nexary should cancel or reject it before business work starts and record the result in local diagnostics and observation events. The Sentinel provider is planned for `0.12.x`; retry stop propagation across retry chains is planned for `0.13.x`.
...
The current source version is `0.11.0`. If Maven Central has not synced this version yet, build from GitHub tag `v0.11.0` and install it locally:
...
- Use a GitHub Releases / Tags version. For example, tag `v0.11.0` maps to dependency version `0.11.0`.
...
<properties>
  ...
  <nexary.version>0.11.0</nexary.version>
</properties>
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot2-starter:0.11.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot2-starter:0.11.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot4-starter:0.11.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot4:0.11.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot4-starter:0.11.0'
}
```
...
### 11. non-starter dependency mode
...
dependencies {
    ...
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot2:0.11.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot2:0.11.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot2:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot2:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot2:0.11.0'
}
```
...
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot4:0.11.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot4:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot4:0.11.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot4:0.11.0'
}
```
...
- `0.11.0` includes verified entries for Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21
