...
The current `0.14.0` line uses Spring Boot 3.3 / Java 17+ as the mainline. The verified capabilities include cache, messaging, jobs, observation bridging, the local governance runtime, the read-only governance diagnostic Console, the Boot3 Sentinel provider, retry-stop propagation, and isolation by fixed traffic class and priority. The governance sample shows resources, policy snapshots, runtime snapshots, recent events, slow-call records, open circuit, half-open probes, recovery, Sentinel block reasons, retry stop reasons, traffic class, priority, and isolation reason. Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21 entries are still provided per verified module.

The `0.14.0` line does not replace Sentinel Dashboard, cluster flow control, or remote rule platforms. It lets online requests, offline tasks, batch work, and background repair work share the same resource while lower-priority traffic is rate-limited, isolated, or sent to fallback first. Business code still calls Nexary APIs. The local engine and the Boot3 Sentinel provider both read the same priority policy, while Nexary owns low-cardinality diagnostics, samples, and the read-only Console. Boot2 / Boot4 Sentinel provider support is not in the support matrix yet.
...
The current source version is `0.14.0`. If Maven Central has not synced this version yet, build from GitHub tag `v0.14.0` and install it locally:
...
- Use a GitHub Releases / Tags version. For example, tag `v0.14.0` maps to dependency version `0.14.0`.
...
<properties>
  ...
  <nexary.version>0.14.0</nexary.version>
</properties>
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot2-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot2-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot2-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-governance-gateway-spring-boot2-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot4-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot4-starter:0.14.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot4:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot4-starter:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-api:0.14.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot2:0.14.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot2:0.14.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot2:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot2:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot2:0.14.0'
}
```
...
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot4:0.14.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot4:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot4:0.14.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot4:0.14.0'
}
```
...
- `0.14.0` includes verified entries for Spring Boot 2.7 / Java 8+ and Spring Boot 4.1 / Java 21
