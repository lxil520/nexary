  ...
  return computed(() => {
    ...
    return resources.value.filter((resource) => {
      ...
      const matchesKeyword =
        keyword.length === 0 ||
        normalize(
          `${resource.resourceKey} ${resource.kind} ${resource.name} ${resource.provider} ${resource.operation} ${resource.priority}`,
        ).includes(keyword);
      ...
      return matchesKeyword && matchesKind && matchesCircuit && matchesProvider;
    });
  });
}
  ...
  return computed(() => {
    ...
    return events.value.filter((event) => {
      const matchesKeyword =
        keyword.length === 0 ||
        normalize(
          `${event.resourceKey} ${event.action} ${event.outcome} ${event.rejectionReason} ${event.circuitState} ${event.durationBucket}`,
        ).includes(keyword);
