...
class PowerJobBoot2AutoConfigurationTest {
    private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
            ...
            .withPropertyValues("nexary.job.provider=powerjob");

    @Test
    void createsBridgeOperationsForBoot2Line() {
        contextRunner.run(context -> {
            assertThat(context).hasSingleBean(PowerJobBridge.class);
            ...
            JobExecutionRecord record = operations.triggerExecution("boot2-powerjob-job", 2, 5);
    ...
    static class JobConfiguration {
        @Bean
        NexaryJob boot2PowerJob() {
            return new NexaryJob() {
                @Override
                public String name() {
                    return "boot2-powerjob-job";
                }
