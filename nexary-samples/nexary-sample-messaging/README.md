...
这个样例演示 Spring Boot 服务怎么接 Nexary Messaging。业务代码只写发送入口和消费者，Kafka/Redis/RocketMQ/Disruptor 由配置选择。
...
样例业务代码只使用 Nexary messaging API。Boot3、Boot2 Redis-only、Boot4 的差异只在依赖和配置。现在开发版是 `0.2.0-SNAPSHOT`；发布到 Maven Central 后，把示例中的版本换成最新 release。

| Spring Boot | JDK | 样例依赖入口 | Provider 边界 |
| --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | `nexary-messaging-spring-boot-starter` | starter selector 选择 `disruptor` / `redis` / `kafka` / `rocketmq` |
| Spring Boot 2.7 | Java 8+ | `nexary-messaging-spring-boot2-starter` | Redis-only 已验证 |
| Spring Boot 4.1 | Java 21 作为 Nexary 主要验证运行时 | `nexary-messaging-spring-boot4-starter` + 恰好一个 `*-spring-boot4` provider | 按 provider 验证；不提供全 provider 聚合 starter |

## 常用接法：starter

业务代码只依赖 Nexary messaging API。切换 provider 时改 `nexary.messaging.provider`、provider 配置和依赖，不改 controller、发送入口或 consumer。
...
Boot4 starter 不会把所有 provider 都放进 classpath。Spring Boot 4 官方最低 JDK 以 Spring 官方文档为准；这里的 Java 21 只是 Nexary 验证 Boot4 的运行时。

如果使用 Redis profile，需要 Spring Redis 连接工厂；Kafka/RocketMQ profile 需要对应 broker。
...
- `org.nexary.samples.messaging.api`：HTTP/测试触发入口，只依赖 sample 发送入口
- `org.nexary.samples.messaging.facade`：可复制到业务侧的发送入口，只依赖 Nexary messaging API
...
业务 consumer 抛出异常时，样例代码不处理 provider 原生 retry。Nexary messaging 通过 `MessageRetryPolicy` 控制 `retry-max-attempts`、`retry-initial-delay`、`retry-backoff-strategy`、`retry-max-backoff`。重试耗尽后写入 `MessageDeadLetterRecord`，默认由内存版 `MessageDeadLetterPublisher` 记录。
...
这些模块只依赖 `nexary-messaging-api` 和一个具体 provider 模块。业务代码仍然只使用 Nexary messaging API；切换 provider 是切换依赖模块和配置，不改 controller、发送入口或 consumer。SPI 样例包名按 provider 隔离，例如 `org.nexary.samples.messaging.spi.kafka.*`。
