...
这是消息样例，重点展示业务代码如何用同一套 Nexary API 发送和消费消息，并通过配置切换 Disruptor、Redis queue、Kafka、RocketMQ、ActiveMQ Classic。
...
样例业务代码只使用 Nexary messaging API，因此 Boot3、Boot2 Redis-only、Boot4 按 provider 引入的差异只在依赖和配置入口。开发版本示例统一使用 `0.7.0`；发布到 Maven Central 后，把示例中的版本替换成最新 release。
...
| Spring Boot 4.1 | Java 21 作为 Nexary 主要验证运行时 | `nexary-messaging-spring-boot4-starter` + 恰好一个 `*-spring-boot4` provider | 按 provider 独立引入；不提供 all-provider aggregate starter |
...
这些模块只依赖 `nexary-messaging-api` 和一个具体 provider 模块。业务代码仍然只使用 Nexary messaging API；切换 provider 是切换依赖模块和配置，不改 facade/controller/consumer。样例包名按 provider 隔离，例如 `org.nexary.samples.messaging.spi.kafka.*`。

发布后不用 starter 的依赖形态：

Spring Boot 3.3 / Java 17+ 按 provider 四选一，每个代码块都可以直接复制。
...
Spring Boot 4.1 / Java 21 主要验证运行时按 provider 引入：
...
先看 `result.status`。`SUCCESS` 表示 provider 已接收；`FAILED` 会在 `result.detail` 和 `result.retrySignal` 里给出原因，例如 publish deadline 已过期。
...
| 字段 | 说明 |
| --- | --- |
| `published[].messageId` | 发送时传入或自动生成的业务消息 id。 |
| `published[].publishStatus` | 发送结果，通常是 `SUCCESS` 或 `FAILED`。 |
| `published[].providerMessageId` | provider 返回的消息 id；不同 provider 可能为空。 |
| `published[].detail` | 失败或 provider 返回的简短说明。 |
| `consumed[]` | 当前进程消费到的消息快照，包含 payload 和 `consumedAt`。 |

真实中间件 profile 也用同一组 curl 命令，只换启动命令：
...
消费端订阅的是业务常量 `MessagingSampleTopics.APP_ERROR_LOG`，不是 provider 配置项。这个 topic 使用 RocketMQ 也合法的 `sample_messaging_app_error_log`，因此 Kafka / RocketMQ / Redis / Disruptor / ActiveMQ Classic 切换时不改业务代码。
