...
这个样例演示 Spring Boot 服务怎么接 Nexary Cache。业务代码只用 `CacheClient` / `CacheCounterClient`，Redis 由 starter 和 `nexary.cache.*` 配置接上。
...
当前可运行样例使用 Spring Boot 3.3.x + Java 17+。如果你用 Boot2 或 Boot4，业务代码不变，只换 starter。
...
starter 带上 cache API 和 provider。业务代码只注入 `CacheClient` / `CacheCounterClient`，切换 provider 时不改 controller/service。
