...
当前样例对应 Spring Boot 3.3.x + Java 17+ 已验证线。它不使用 starter，而是展示 API + Redis provider 依赖模式。

| Spring Boot | JDK | 状态 | API / provider 依赖 |
| --- | --- | --- | --- |
| Spring Boot 3.3.x | Java 17+ | 当前已验证 | `nexary-cache-api` + `nexary-cache-redis` |
| Spring Boot 2.7.x | Java 8+ | v0.2 目标，待验证，未发布 | 拟定 Java8 兼容 API/provider 线 |
| Spring Boot 4.x | Java 21+ | v0.2 后续目标，待验证，未发布 | 拟定 Boot4 provider 线 |
