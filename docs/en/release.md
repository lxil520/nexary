...
This page is the public release runbook. Examples use `0.12.0`; replace the version everywhere when cutting another patch release.
...
- run the full command from "Build a Central Bundle Locally" with `./gradlew mavenCentralBundle -PnexaryVersion=0.12.0`, then confirm `build/distributions/nexary-0.12.0-central-bundle.zip` exists
...
If the Central token is missing, the publish step fails instead of marking a tag run successful without publishing to Central. For a bundle-only check, run `workflow_dispatch`, enter `0.12.0` or `v0.12.0`, and keep `publish_to_central=false`. Manual Central publication must run from an existing tag ref, and the entered version must match the selected tag; do not publish a Central deployment manually from the `main` branch.
