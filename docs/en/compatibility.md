...
- The Boot 2 compatibility line primarily serves Java 8 users. Cache, Messaging, and Job handle Java 17+ syntax, Java 9+ APIs, dependency versions, and auto-configuration entry-point differences separately.
...
2. Decide the Java 8 compatibility strategy per module: migrate the current public APIs or split a Java 8-compatible API / adapter. A module cannot declare Java 8 support while its public APIs still expose `record`.
...
4. Messaging Disruptor/Kafka/RocketMQ Boot2/JDK8 providers still require independent gates. Boot4/JDK21 has verified Cache Redis, Messaging provider-by-provider, and bounded Job entries, but a dedicated Boot4 BOM, samples, and release closeout still need to progress.
