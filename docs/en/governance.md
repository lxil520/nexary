...
The boundary is deliberate: the local governance runtime does not provide a sidecar, agent, remote config push, or cross-instance state sync. The governance platform aggregates resources, signals, topology, incident candidates, review material, and local connector metadata. In v0.24, the only writes are Nexary-local plans, connector configuration, service mappings, notification-test results, and audit records. It does not modify Sentinel, Gateway, APM, registry, or production notification-channel configuration.

The `0.24.0` line does not replace Sentinel Dashboard, Spring Cloud Gateway, SkyWalking, CAT, Prometheus, enterprise IM, automatic traffic-drain platforms, or a distributed trace backend. It handles two layers: the local governance runtime still handles request cancellation, the Sentinel provider, retry-stop, priority isolation, abnormal instance candidates, and local fault traces; the governance platform aggregates services, clusters, zones, middleware dependencies, request-flow samples, transaction stats, host watermarks, and low-cardinality signals into read-only topology, incident evidence packages, governance plans, dry-run results, notification previews, connector configuration, service mappings, and audit records. v0.18 groups slow calls, error-rate signals, Sentinel blocks, Gateway disconnects, retry-stop signals, and abnormal instance signals into incident candidates. v0.19 turns Platform Mode into an operations workbench draft. v0.20 makes the default Console entry a full governance platform RC with Overview, Topology, Request Flows, Incidents, Services, Hosts, Middleware, Resource Governance, Integrations, Notifications, and Policy Plans. v0.21 moves the sample into a real Docker verification environment with Redis, Postgres, RabbitMQ, Prometheus, and SkyWalking. v0.22 completes the trusted read-only platform data source, freshness, trace query, service, host, and integration views. v0.23 generates review material and test notifications only; it does not push production configuration. v0.24 adds the connector configuration center, explicit TEST / DRY-RUN probes, and mappings from services to external tool objects; it still does not write Sentinel rules, change Gateway routes, or send production alerts. The v0.11 cancellation check still runs before Sentinel entry, so canceled requests do not pollute Sentinel windows. v0.15 instance health records only real downstream results and does not count Sentinel blocks as instance failures. v0.16 traces store only low-cardinality fields and do not store business parameters. Platform signals also reject user ids, tenants, payloads, cache keys, message ids, exception text, stack traces, tokens, and passwords.
...
## v0.24 Connector Configuration Center

v0.24 adds the entry point for connecting governance tools to the platform. It introduces local connector configuration, explicit connector tests, and service mappings:

- `POST /api/platform/connector-configs` stores local metadata for SkyWalking, Spring Cloud Gateway, Sentinel, Prometheus, Actuator, Feishu, DingTalk, or Webhook.
- `POST /api/platform/connector-configs/{connectorKey}/test` performs only explicit TEST / DRY-RUN probes. Non-notification connectors use a GET reachability check. Notification connectors send a TEST-marked message only when `testEnabled=true`.
- `POST /api/platform/service-mappings` stores mappings between Nexary service keys and external tool object keys so services, routes, Sentinel resources, SkyWalking services, and notification routes can be connected.

These records still use `nexary-governance-platform-storage-jdbc`, with PostgreSQL, MySQL/MariaDB, and SQLite dialects. Responses hide the path part of connector endpoints, and the frontend will not save a masked endpoint back as a real URL. Every button in the connector center performs a real local action, but the action boundary remains local configuration, test, mapping, and audit. External production systems are not written.
