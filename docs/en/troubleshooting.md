...
Make sure all copied snippets use one version. GitHub tag `v0.8.0` maps to dependency version `0.8.0`.
...
dependencies {
    ...
    runtimeOnly "com.aweimao:nexary-messaging-redis:0.8.0"
}
```
...
```groovy
implementation "com.aweimao:nexary-observation-micrometer-spring-boot-starter:0.8.0"
```
...
```bash
./gradlew mavenCentralBundle -PnexaryVersion=0.8.0
```
