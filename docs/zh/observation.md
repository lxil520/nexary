...
## v0.16 fault trace 和指标的关系

`/nexary/governance/traces` 和 Console Trace detail 是本地 diagnostics 视图，不是 Micrometer 指标，也不是外部 trace exporter。`traceKey` 只用于本地查询一条 trace，不会进入 `nexary.observation.events.total` 或 `nexary.observation.events.duration` 的 tag。
...
指标看“多少次、耗时如何、按固定标签怎么分布”；trace 看“某次本地调用为什么停、建议先查哪个 resource”。两者使用不同出口。

## 非目标

- 不做 Jaeger、Zipkin、SkyWalking 或 OpenTelemetry exporter。
- 不做 audit log 或安全审计。
- 不声明事件 exactly-once。
