...
它的边界很明确：本地治理运行时不做 sidecar、agent、远程下发配置或跨实例状态同步。治理平台只做资源、信号、拓扑、事故候选、审查材料和本地连接器元数据的汇聚；v0.24 允许写入的内容只限 Nexary 本地计划、连接器配置、服务映射、通知测试结果和审计记录，不修改 Sentinel、Gateway、APM、注册中心或生产通知渠道配置。

`0.24.1` 不替代 Sentinel Dashboard、Spring Cloud Gateway、SkyWalking、CAT、Prometheus、企业 IM、自动摘流平台或分布式 trace 后端。它解决两层问题：本地治理运行时继续处理请求取消、Sentinel provider、停止重试、优先级隔离、异常实例候选和本地 fault trace；治理平台把多个 JVM 或 connector 上报的服务、集群、机房、中间件依赖、请求链路样本、交易统计、主机水位和低基数信号汇总成只读拓扑、事故证据包、治理计划、dry-run 结果、通知预览、连接器配置、服务映射和审计记录。v0.18 会把同一服务、集群、机房内的慢调用、错误率、Sentinel block、Gateway 断开、停止重试和异常实例信号聚合成事故候选。v0.19 把 Platform Mode 改成运维工作台雏形。v0.20 把默认 Console 入口改成完整治理平台 RC，主路径包含总览、拓扑、请求链路、事故、服务、主机实例、中间件、资源治理、集成、通知和策略计划。v0.21 把 sample 推进到 Docker 中的 Redis、Postgres、RabbitMQ、Prometheus 和 SkyWalking 真实验证环境。v0.22 补齐可信只读平台的数据来源、新鲜度、Trace 查询、服务/主机/集成视图。v0.23 只生成审查材料和测试通知，不下发生产配置。v0.24 增加连接器配置中心、显式 TEST / DRY-RUN 探测和服务到外部工具对象的映射；仍不写 Sentinel 规则、不改 Gateway route、不发送生产告警。v0.11 的请求失效终止仍在 Sentinel entry 前检查，已取消请求不会进入 Sentinel 统计窗口；v0.15 的实例健康只记录真实下游结果，不把 Sentinel block 当成实例故障；v0.16 trace 只保存低基数字段，不保存业务参数；平台信号也拒绝 userId、tenant、payload、cache key、message id、异常全文、stack trace、token 和密码。
...
- `nexary-governance-platform-storage-jdbc`：显式接入的 JDBC repository，供需要持久化平台证据的部署使用，支持 PostgreSQL、MySQL/MariaDB 和 SQLite 方言；demo 默认仍使用内存存储。
- `nexary-governance-platform-storage-postgres`：兼容旧接入方式的 artifact，内部固定使用 PostgreSQL 方言。
...
v0.21 开始把治理平台 sample 从静态演示推进到可验证的本地链路。运行完整 Docker 环境：
...
v0.24 解决“治理平台如何跟外部工具打通”的入口问题。平台新增本地连接器配置、显式连接测试和服务映射 API：

- `POST /api/platform/connector-configs` 保存 SkyWalking、Spring Cloud Gateway、Sentinel、Prometheus、Actuator、Feishu、DingTalk 或 Webhook 的本地元数据。
- `POST /api/platform/connector-configs/{connectorKey}/test` 只做显式 TEST / DRY-RUN 探测。非通知连接器发起 GET 可达性检查；通知连接器只有 `testEnabled=true` 时才发送标记为 TEST 的消息。
- `POST /api/platform/service-mappings` 保存 Nexary service key 和外部工具对象 key 的映射，便于把服务、route、Sentinel resource、SkyWalking service 和通知路由串起来。

这些配置继续走 `nexary-governance-platform-storage-jdbc`，支持 PostgreSQL、MySQL/MariaDB 和 SQLite 方言。响应会隐藏 connector endpoint 的路径部分；前端不会把脱敏 endpoint 当成真实地址再次保存。连接器中心的按钮都有真实动作，但动作边界仍是本地配置、测试、映射和审计，不会写外部生产系统。
