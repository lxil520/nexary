...
- 当前 `0.3.x` release 只声明已经通过 gate 的组合；当前已验证组合包括 Spring Boot 3.3 / Java 17+ 主线，以及 Spring Boot 2.7 / Java 8+ 的 Cache Redis/Valkey 单级缓存入口、Messaging Redis-only 入口和 Job local/XXL-JOB/PowerJob bridge 入口。
...
- `0.3.x`：Java 17 / Spring Boot 3.3 主线发布，加上 Boot2/JDK8 和 Boot4/JDK21 模块入口的可执行 gate。
- `0.3.x` 后续：继续补齐 Boot2/JDK8 Messaging 的 Disruptor/Kafka/RocketMQ/ActiveMQ Classic provider，并按真实样例和集成测试逐项扩展支持声明。
