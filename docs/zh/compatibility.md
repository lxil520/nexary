...
| Spring Boot 4.1 + Java 21 | Cache Redis provider / starter 已验证；Messaging 按 provider 已验证且 starter 不会自动带上所有 provider；Job provider / starter 受限边界已验证；不是全仓库 Boot4 支持；官方最低 JDK 仍以 Spring 文档为准 |
...
- Boot 2 兼容线主要服务 Java 8 用户。Cache、Messaging、Job 分开处理 Java 17+ 语法、Java 9+ API、依赖版本和自动配置入口差异。
...
2. 分模块决定 Java 8 兼容策略：改造当前公共 API，还是拆出 Java 8 兼容 API / adapter。未解决公共 API `record` 前，对应模块不能声明 Java 8 用户可用。
...
4. Messaging Disruptor/Kafka/RocketMQ 的 Boot2/JDK8 provider 仍需独立 gate；Boot4/JDK21 已验证 Cache Redis、Messaging provider-by-provider 和 Job 受限边界入口，但 Boot4 专用 BOM、样例和发布收口仍需继续推进。
