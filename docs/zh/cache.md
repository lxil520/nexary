...
当前开发版本：`0.12.0`。正式发布后，依赖中的 `${nexary.version}` 使用最新 release / tag 版本。
...
```groovy
dependencies {
    implementation "com.aweimao:nexary-cache-api:0.12.0"
    runtimeOnly "com.aweimao:nexary-cache-redis-spring-boot2:0.12.0"
}
```
...
dependencies {
    implementation "com.aweimao:nexary-cache-api:0.12.0"
    runtimeOnly "com.aweimao:nexary-cache-redis-spring-boot4:0.12.0"
}
```
