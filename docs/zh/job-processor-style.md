...
Processor-style 是面向生产任务执行进程的接入形态。它和 focused adoption sample 不同：
...
两者共享 `NexaryJob` 抽象，但使用场景不同。不要把 starter selector sample 当成 processor 生产形态，也不要把 processor skeleton 当成完整外部平台联调结论。

focused sample 的 `business/SampleBusinessJob` 展示了更明确的 adoption 形态：业务 handler 只实现 `NexaryJob`，切换本地调度和 XXL-JOB bridge 不修改业务代码。
...
Nexary 的 processor-style 集成只暴露 Nexary 自己的 job API。历史项目里的 handler 类型、参数类型和命名不会进入 Nexary 公共 API。
...
- 不在当前版本实现 PowerJob bridge
- 不把 processor skeleton 包装成完整外部平台联调结论
