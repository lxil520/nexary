...
后续优先把已有治理信号做成能值班使用的平台，不急着进入容量、混沌和自动止损。
...
- `0.20.x`：统一治理平台 RC，默认入口改成总览、拓扑、请求链路、事故、服务、主机实例、中间件、资源治理、集成、通知和策略计划；补云手机 demo、请求链路样本、CAT 式交易统计、主机信号矩阵和只读平台 snapshot。
- `0.21.x`：Prometheus / Micrometer 指标和服务水位真实只读接入，把 v0.20 demo 水位换成可配置数据源。
- `0.22.x`：Sentinel、Gateway、OTel / SkyWalking、Actuator / Spring Boot Admin 的只读 connector 和证据链升级，把外部引用变成真实跳转。
- `0.23.x`：策略计划 dry-run，只生成 diff 和审查材料，不写外部系统。
- `0.24.x`：Feishu / DingTalk 通知通道 dry-run、测试发送、去重、静默和恢复通知规则。
- `0.25.x`：平台 UI 深化和真实大盘配置，支持更多服务详情、图表组合、收藏视图和团队视图。
- `0.26+`：再评估 RBAC、审计、策略版本、回滚、保留策略、容量、混沌、自动止损、自动摘流和自动扩容。
