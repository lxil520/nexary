...
Job 先分清两个入口：本地 scheduler 自己按 cron 触发；XXL-JOB bridge 接收外部平台触发。业务代码都只写 `NexaryJob`。
...
用 local scheduler 时，cron 写在 `application.yml`：
...
`job-name` 必须等于业务类 `NexaryJob.name()`。也可以注入 `NexaryJobOperations`，在代码里调用 `schedule(new JobSchedule(...))`。两种写法后面走同一套本地调度逻辑。
...
- PowerJob：以后可以按 bridge 接，不在现在的实现里

## 怎么划边界

- `NexaryJob` 是统一任务抽象。
- 本地调度由 Nexary 自己按 cron 执行。
- XXL-JOB 只负责把外部平台触发转成 `NexaryJob` 执行，不是第二套公开 job API。
- 以后接 PowerJob，也应保持这个方向：接入平台触发，不反过来改业务 job API。
...
| Spring Boot 2.7.x | Java 8+ | 已验证受限边界 | `org.nexary:nexary-job-spring-boot2-starter` |
| Spring Boot 4.1.x | Java 21 是 Nexary 主验证运行时；官方最低 JDK 以 Spring 官方文档为准 | 已验证 Boot4 provider/starter | `org.nexary:nexary-job-spring-boot4-starter` |

Boot3 starter 叫 `nexary-job-spring-boot-starter`。Boot2 和 Boot4 用显式名字：`nexary-job-spring-boot2-starter`、`nexary-job-spring-boot4-starter`。换 starter 或 provider，不应该改业务 job 代码。
...
Nexary v0.1 把直接触发、本地定时和 XXL-JOB bridge 触发放进同一套执行记录：
...
需要接 Prometheus 或其他 Micrometer 后端时，引入 `nexary-observation-micrometer-spring-boot-starter`。这个 starter 只把 Nexary 事件转成 Micrometer meter，不改变 `NexaryJob`、scheduler 或 XXL-JOB bridge 的 public API。自定义企业指标平台仍可提供 `NexaryObservationListener`，转换时必须继续遵守上面的标签白名单。
...
- 直接执行业务 job
- 通过 `nexary.job.scheduler.schedules` 或 `NexaryJobOperations.schedule(...)` 注册 cron 调度
...
2. 用 `nexary-sample-job` 跑 starter 接入
