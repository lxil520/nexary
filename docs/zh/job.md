...
最常见的 local scheduler 用法直接写 `application.yml`：
...
`job-name` 必须等于业务类 `NexaryJob.name()`。如果你更喜欢代码注册，也可以注入 `NexaryJobOperations` 调用 `schedule(new JobSchedule(...))`。两种方式进入同一条 local scheduler 执行管线。
...
| Boot2 XXL-JOB bridge | `nexary-job-xxljob-spring-boot2` | 已验证 bridge-shaped 入口 |
...
| Boot4 XXL-JOB bridge | `nexary-job-xxljob-spring-boot4` | 已验证 bridge-shaped 入口 |
