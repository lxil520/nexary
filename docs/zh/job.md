...
- PowerJob：后续 bridge 方向，不属于当前已实现能力
...
- PowerJob 后续也应按 bridge 接入，不能倒逼修改当前公共 API。

## 版本入口与依赖选择

当前 Job 能力只声明已验证组合，不把目标组合写成已支持。

| Spring Boot | JDK | 状态 | 推荐入口 |
| --- | --- | --- | --- |
| Spring Boot 3.3.x | Java 17+ | 当前已验证 | `org.nexary:nexary-job-spring-boot-starter` |
| Spring Boot 2.7.x | Java 8+ | v0.2 兼容目标，待验证，未发布 | 拟定 `org.nexary:nexary-job-spring-boot2-starter` |
| Spring Boot 4.x | Java 21+ 主验证目标 | v0.2 后续验证目标，待验证，未发布 | 拟定 `org.nexary:nexary-job-spring-boot4-starter` |

当前已验证 starter artifactId 是 `nexary-job-spring-boot-starter`。如果项目决定同时发布 Boot2 / Boot3 / Boot4 多条线，发布前应做最小命名调整：把当前 Boot3 入口显式化为 `nexary-job-spring-boot3-starter`，再把未验证的 Boot2 / Boot4 入口保留为独立目标 artifact。这样用户不会把 Boot3 starter 误用于 Boot2。
...
SPI/provider 模式适合只引入一个具体 provider。业务 job 仍只依赖 `NexaryJob`、`JobContext`、`JobResult`：
...
// XXL-JOB bridge provider。二选一：不要和 local scheduler 隐式混用。
// runtimeOnly 'org.nexary:nexary-job-xxljob'

// 可选：Redis durable execution store。
// 只有需要跨进程/重启后查询 completed execution record 时才引入并启用。
// runtimeOnly 'org.nexary:nexary-job-execution-store-redis'
```

目标但未发布的兼容 artifact 命名：

| 目标 | 拟定 artifactId | 状态 |
| --- | --- | --- |
| Boot2 starter | `nexary-job-spring-boot2-starter` | 目标 / 待验证 / 未发布 |
| Boot2 Java8 API | `nexary-job-api-java8` | 目标 / 待验证 / 未发布 |
| Boot2 local scheduler | `nexary-job-scheduler-spring5` | 目标 / 待验证 / 未发布 |
| Boot2 XXL-JOB bridge | `nexary-job-xxljob-spring5` | 目标 / 待验证 / 未发布 |
| Boot2 Redis execution store | `nexary-job-execution-store-redis-spring5` | 目标 / 待验证 / 未发布 |
| Boot4 starter | `nexary-job-spring-boot4-starter` | 目标 / 待验证 / 未发布 |
...
local scheduler、XXL-JOB bridge 和 processor 启动方式属于 starter/provider infrastructure。应用可以先使用 `nexary-job-spring-boot-starter` 聚合当前 provider，也可以只依赖 `nexary-job-api`，再按运行模式引入一个具体 provider。

切换 local scheduler 和 XXL-JOB bridge 时，不应修改业务 job handler。starter 模式只改 `nexary.job.provider` 和相关配置；SPI/provider 模式只改 provider 依赖和相关配置。
...
Nexary v0.1 的 job 执行生命周期是 provider-neutral 的：
...
`triggerExecution(...)` 返回完整执行记录；旧的 `trigger(...)` 仍可用于只关心 `JobResult` 的简单场景。direct trigger、local cron schedule 和 XXL-JOB bridge 触发都进入同一条执行管线，因此监听、重试、超时、结果映射和执行记录语义一致。
...
- `capability`：固定为 `job`
- `operation`：事件名
- `provider`：`local`、`xxljob`、`memory`、`redis` 或 `unknown`
...
需要接 Prometheus 或其他 Micrometer 后端时，引入 `nexary-observation-micrometer-spring-boot-starter`。该 bridge 只消费 provider-neutral event 并生成 Micrometer meter，不改变 `NexaryJob`、scheduler 或 XXL-JOB bridge 的 public API。自定义企业指标平台仍可提供 `NexaryObservationListener`，转换时必须继续遵守上面的标签白名单。
