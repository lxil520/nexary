...
这份文档是面向 Nexary 维护者和贡献者的主规范。目标不是增加流程，而是保证公共 API、样例、文档、测试能长期收敛。
...
- 每个 capability 都要有独立验收面，不靠全量 showcase 代替
- 验证过程只核验事实，不替能力设计补方案

## 禁止项

- 不引入旧项目名、公司名、个人名、内网地址、真实凭证
- 不为了短期兼容把主线 API 做乱
- 不在 `0.1.x` 混入 sidecar、agent、控制面、后台平台类能力
- 不在文档里夸大未验证过的 provider、样例或联调结论
