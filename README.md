...
当前 `0.11.1` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力仍是缓存、消息、任务调度、可观测性桥接、本地治理运行时和只读治理诊断 Console。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测和恢复过程；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 按模块提供已验证入口。

`0.11.1` 不替代 Sentinel，也不提供远程治理平台。它解决的窄问题是 stale request cancellation：当 deadline 已经过期，或者请求在进入本地治理前已经不值得启动时，Nexary 应在业务动作开始前取消或拒绝，并把结果记录到本地诊断和观测事件里。Sentinel provider 放在 `0.12.x`，跨 retry 链路传播停止信号放在 `0.13.x`。
...
当前源码版本是 `0.11.1`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.11.1` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.11.1` 对应依赖版本 `0.11.1`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.11.1`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter`<br>`nexary-governance-gateway-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge、Gateway cancellation 已验证 | `0.11.1`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter`<br>`nexary-governance-gateway-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.11.1`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
- `0.11.1` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
