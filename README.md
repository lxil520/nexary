...
当前 `0.12.0` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力包括缓存、消息、任务调度、可观测性桥接、本地治理运行时、只读治理诊断 Console，以及 Boot3 主线的 Sentinel provider。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测、恢复过程、Sentinel 拦截原因和当前执行引擎；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 仍按模块提供已验证入口。

`0.12.0` 不替代 Sentinel Dashboard、集群限流或远程规则平台。它做的是让 Nexary 本地治理资源可以选择交给 Sentinel 执行：业务代码仍调用 Nexary API，QPS 限流、线程数隔离、慢调用熔断和异常熔断由 Sentinel 判断，Nexary 负责稳定接入、低基数诊断、样例和只读 Console。Boot2 / Boot4 的 Sentinel provider 还没有写入支持矩阵；跨 retry 链路传播停止信号放在 `0.13.x`。
...
当前源码版本是 `0.12.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.12.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.12.0` 对应依赖版本 `0.12.0`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.12.0`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter`<br>`nexary-governance-gateway-spring-boot-starter`<br>`nexary-governance-sentinel-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge、Gateway cancellation 已验证 | `0.12.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter`<br>`nexary-governance-gateway-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.12.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
- `0.12.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
