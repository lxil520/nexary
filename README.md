...
当前 `0.8.x` 以 Spring Boot 3.3 / Java 17+ 为主线，已经覆盖缓存、消息、任务调度、可观测性桥接和本地治理运行时。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测和恢复过程；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 按模块提供已验证入口。

`0.8.x` 把本地治理数据面补到可复制验证。你可以启动 `nexary-sample-governance`，先用 curl 触发成功、失败、限流、并发隔离和熔断，再访问 `/nexary/governance/summary`、`/nexary/governance/resources`、`/nexary/governance/events` 查看低基数字段。它不是控制台、sidecar、agent 或远程配置平台。
...
为了扩大使用人群，Nexary 把 Spring Boot 2.7 / Java 8+ 和 Spring Boot 4.1 / Java 21 纳入 `0.8.x` 验证目标。README 只给已经通过样例和测试的依赖片段；没有通过验证的组合不会写成“支持”。Java 21 是 Nexary 对 Boot4 线的主验证运行时，不代表 Spring Boot 4 的官方最低 JDK；官方最低 JDK 仍以 Spring 官方文档为准。
...
当前源码版本是 `0.8.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.8.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.8.0` 对应依赖版本 `0.8.0`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.8.0`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge 已验证 | `0.8.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.8.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
<properties>
  ...
  <nexary.version>0.8.0</nexary.version>
</properties>
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot2-starter:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot2-starter:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot2-starter:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-cache-spring-boot4-starter:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-spring-boot4-starter:0.8.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot4:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-spring-boot4-starter:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-messaging-api:0.8.0'
    runtimeOnly 'com.aweimao:nexary-messaging-redis-spring-boot2:0.8.0'
}
```
...
```groovy
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot2:0.8.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot2:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot2:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot2:0.8.0'
}
```
...
dependencies {
    implementation 'com.aweimao:nexary-job-api:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-scheduler-spring-boot4:0.8.0'
}
```
...
```groovy
dependencies {
    runtimeOnly 'com.aweimao:nexary-job-xxljob-spring-boot4:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-powerjob-spring-boot4:0.8.0'
    runtimeOnly 'com.aweimao:nexary-job-execution-store-redis-spring-boot4:0.8.0'
}
```
...
- `0.8.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
