...
当前 `0.22.0` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力包括缓存、消息、任务调度、可观测性桥接、本地治理运行时、只读治理诊断 Console、Boot3 主线的 Sentinel provider、停止重试传播、按固定流量类型和优先级做隔离、异常实例候选识别，以及当前 JVM 内的本地故障定位 trace。平台方向已包含资产模型、资源/信号上报 API、服务/依赖拓扑查询、Postgres 存储边界、Console Platform Mode，以及 v0.18 的事故证据链：同一服务、集群、机房内的慢调用、错误率、Sentinel block、Gateway 断开、停止重试和异常实例信号会聚合成可读的事故候选。`0.22.0` 把治理平台样例接入 Docker 中的 Redis、Postgres、RabbitMQ、Prometheus 和 SkyWalking，能跑真实读写、发布消费、Prometheus 指标采集和 SkyWalking agent 链路写入。Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 仍按模块提供已验证入口。

`0.22.0` 不是 Sentinel、Spring Cloud Gateway、SkyWalking、Prometheus、企业 IM 或 OpenTelemetry 后端的替代品，也不修改外部工具规则。它把 Nexary SDK 资源、治理信号、服务组、集群、机房、中间件依赖和只读连接器状态放到同一个平台视图里，并让事故候选带上首要资源、影响资源数、证据时间线和建议检查项。Local Console 仍用于单 JVM 调试；Platform Mode 用于看汇聚后的服务、依赖、事故队列、拓扑影响和证据入口。payload、URL query、userId、tenant、messageId、cache key、异常全文、stack trace、token 和密码都不能进入平台信号。
...
当前源码版本是 `0.22.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.22.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.22.0` 对应依赖版本 `0.22.0`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.22.0`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter`<br>`nexary-governance-gateway-spring-boot-starter`<br>`nexary-governance-sentinel-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge、Gateway cancellation 已验证 | `0.22.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter`<br>`nexary-governance-gateway-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.22.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
- `0.22.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
