...
当前 `0.11.0` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力仍是缓存、消息、任务调度、可观测性桥接、本地治理运行时和只读治理诊断 Console。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测和恢复过程；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 按模块提供已验证入口。

`0.11.0` 不替代 Sentinel，也不提供远程治理平台。它解决的窄问题是 stale request cancellation：当 deadline 已经过期，或者请求在进入本地治理前已经不值得启动时，Nexary 应在业务动作开始前取消或拒绝，并把结果记录到本地诊断和观测事件里。Sentinel provider 放在 `0.12.x`，跨 retry 链路传播停止信号放在 `0.13.x`。
...
当前源码版本是 `0.11.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.11.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.11.0` 对应依赖版本 `0.11.0`。
...
- `0.11.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
