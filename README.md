...
当前 `0.14.0` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力包括缓存、消息、任务调度、可观测性桥接、本地治理运行时、只读治理诊断 Console、Boot3 主线的 Sentinel provider、停止重试传播，以及按固定流量类型和优先级做隔离。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测、恢复过程、Sentinel 拦截原因、停止重试原因、流量类型、优先级和隔离原因；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 仍按模块提供已验证入口。

`0.14.0` 不替代 Sentinel Dashboard、集群限流或远程规则平台。它做的是让在线请求、离线任务、批处理和后台修复任务在同一个资源上共存时，低优先级流量先被限流、隔离或走 fallback，避免拖垮在线请求。业务代码仍调用 Nexary API；本地引擎和 Boot3 Sentinel provider 都能读取同一套 priority policy，Nexary 负责低基数诊断、样例和只读 Console。Boot2 / Boot4 的 Sentinel provider 还没有写入支持矩阵。
...
当前源码版本是 `0.14.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.14.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.14.0` 对应依赖版本 `0.14.0`。
...
- `0.14.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
