...
当前 `0.16.0` 以 Spring Boot 3.3 / Java 17+ 为主线，已验证能力包括缓存、消息、任务调度、可观测性桥接、本地治理运行时、只读治理诊断 Console、Boot3 主线的 Sentinel provider、停止重试传播、按固定流量类型和优先级做隔离、异常实例候选识别，以及当前 JVM 内的本地故障定位 trace。治理样例可以看到资源目录、策略快照、运行时快照、最近事件、慢调用统计、熔断打开、半开探测、恢复过程、Sentinel 拦截原因、停止重试原因、流量类型、优先级、隔离原因、异常实例状态、建议动作、trace 停止原因和建议排查资源；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 仍按模块提供已验证入口。

`0.16.0` 不是 Jaeger、Zipkin、SkyWalking 或 OpenTelemetry exporter 的替代品，也不是跨实例 trace 后端。它把当前 JVM 内的治理事件串成只读故障定位视图：一次调用为什么停止、被哪个资源和规则拦住、是否涉及异常实例候选，以及应该先检查哪个 resource。`traceKey` 只用于本地 diagnostics / Console 查询，不进入指标 tag；payload、URL query、userId、tenant、messageId、cache key、异常全文和 stack trace 都不会写入 trace。Boot2 / Boot4 的 Sentinel provider 还没有写入支持矩阵。
...
当前源码版本是 `0.16.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.16.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.16.0` 对应依赖版本 `0.16.0`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.16.0`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter`<br>`nexary-governance-gateway-spring-boot-starter`<br>`nexary-governance-sentinel-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge、Gateway cancellation 已验证 | `0.16.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter`<br>`nexary-governance-gateway-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.16.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
- `0.16.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
