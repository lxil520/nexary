...
当前 `0.7.x` 以 Spring Boot 3.3 / Java 17+ 为主线，已经覆盖缓存、消息、任务调度、可观测性桥接和本地治理运行时。治理样例可以看到慢调用记录、滑动窗口统计、熔断打开、半开探测和恢复过程；Spring Boot 2.7 / Java 8+ 与 Spring Boot 4.1 / Java 21 按模块提供已验证入口。

`0.7.x` 把 messaging publish 治理、runtime 诊断快照和真实中间件样例写到可运行。你可以直接复制依赖，启动 Redis / Kafka / RocketMQ / ActiveMQ Classic profile，用 curl 查看 `result.status`、`published[].publishStatus`、`published[].providerMessageId`、`published[].detail`、`consumed[]`，再用 `/governance/circuit/state` 看本地熔断和拒绝字段。它不是控制台、sidecar、agent 或远程配置平台。
...
为了扩大使用人群，Nexary 把 Spring Boot 2.7 / Java 8+ 和 Spring Boot 4.1 / Java 21 纳入 `0.7.x` 验证目标。README 只给已经通过样例和测试的依赖片段；没有通过验证的组合不会写成“支持”。Java 21 是 Nexary 对 Boot4 线的主验证运行时，不代表 Spring Boot 4 的官方最低 JDK；官方最低 JDK 仍以 Spring 官方文档为准。
...
当前源码版本是 `0.7.0`。如果 Maven Central 还没有同步到这个版本，可以先从 GitHub tag `v0.7.0` 构建并安装到本地：
...
- 使用 GitHub Releases / Tags 中的版本号，例如 `v0.7.0` 对应依赖版本 `0.7.0`。
...
| --- | --- | --- | --- | --- | --- |
| Spring Boot 3.3 | Java 17+ | 当前已验证 | `0.7.0`；Maven Central 同步后也可用 Latest Version | `nexary-bom` | `nexary-cache-spring-boot-starter`<br>`nexary-messaging-spring-boot-starter`<br>`nexary-job-spring-boot-starter`<br>`nexary-observation-micrometer-spring-boot-starter`<br>`nexary-governance-spring-boot-starter` |
| Spring Boot 2.7 | Java 8+ | Cache Redis 单级缓存、Messaging Redis-only、Job local/XXL-JOB/PowerJob bridge 已验证 | `0.7.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot2-starter`<br>`nexary-messaging-spring-boot2-starter`<br>`nexary-job-spring-boot2-starter` |
| Spring Boot 4.1 | Java 21 主验证运行时 | Cache Redis、Messaging 按 provider、Job local/XXL-JOB/PowerJob bridge 已验证；不是全仓库 Boot4 支持 | `0.7.0`；Maven Central 同步后也可用 Latest Version | 当前入口使用直接版本；专用 BOM 另行发布时再切换 | `nexary-cache-spring-boot4-starter`<br>`nexary-messaging-spring-boot4-starter` 加一个 Boot4 provider artifact<br>`nexary-job-spring-boot4-starter` |
...
- `0.7.0` 已纳入 Spring Boot 2.7 / Java 8+、Spring Boot 4.1 / Java 21 的已验证模块入口
